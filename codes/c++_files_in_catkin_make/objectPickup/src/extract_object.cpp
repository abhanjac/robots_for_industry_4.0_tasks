...
// ////////////////////////////////////////////////////////////////////////////////
    ...
    nh.setParam(objectName + "_posiX", output_marker.pose.position.x);
    nh.setParam(objectName + "_posiY", output_marker.pose.position.y);
    nh.setParam(objectName + "_posiZ", output_marker.pose.position.z);
    nh.setParam(objectName + "_oriX", output_marker.pose.orientation.x);
    nh.setParam(objectName + "_oriY", output_marker.pose.orientation.y);
    nh.setParam(objectName + "_oriZ", output_marker.pose.orientation.z);
    nh.setParam(objectName + "_oriW", output_marker.pose.orientation.w);
    nh.setParam(objectName + "_scaleX", output_marker.scale.x);
    nh.setParam(objectName + "_scaleY", output_marker.scale.y);
    nh.setParam(objectName + "_scaleZ", output_marker.scale.z);
    nh.setParam("table_posiZ", table_marker.pose.position.z);
    nh.setParam("table_scaleZ", table_marker.scale.z);
    ROS_INFO("Set storing parameters to with marker details...");

////////////////////////////////////////////////////////////////////////////////

    for (int i=0; i<1 and ros::ok; i++) {
//    while (ros::ok)  {
